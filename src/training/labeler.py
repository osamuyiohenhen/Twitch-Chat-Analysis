...
def main():
    ...
    df = pd.read_csv(INPUT_FILE, header=None, usecols=[0, 1], names=['channel', 'message'], on_bad_lines='skip', engine='python')
    total_rows = len(df)
    ...
    if os.path.exists(OUTPUT_FILE):
        ...
        df_labeled = pd.DataFrame(columns=['channel', 'message', 'label', 'original_index'])
        seen_indices = set()
        ...
        while True:
            ...
            while True:
                ...
                if choice == 'q':
                    ...
                    print("--> 🔴 Negative")
                    # Add to batch
                    new_rows.append({'channel': channel_name, 'message': msg_text, 'label': label, 'original_index': random_idx})
                    seen_indices.add(random_idx)
                    break
